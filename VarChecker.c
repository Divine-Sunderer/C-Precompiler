...
int isValidName(char *var) {
  ...
  for (int i = 0; i < len_res; i++) {
    if (strstr(var, parole_ris[i])) return 0;
  }
...
int varChecker(FILE *principale, char* nome_input, FILE *statistiche) {
  ...
  while (fgets(stringa, sizeof(stringa), principale)) {
    ...
    for (int i = 0; i < index_variabili; i++) {
      ...
      if (isValidName(variabili[i])) {
        printf("FILE: %s var: %s -> VALIDA\n----------------------------------------\n",nome_input, variabili[i]);
        count_valid++;
      }else {
  ...
  //fclose(principale);
  printf("funzione varChecker eseguita con successo\n");
  return 0;
}
